...
context = Context("Edge", bundle="com.microsoft.edgemac")
...
context.keymap(
    {
        ...
        "go tab (1 | 2 | 3 | 4 | 5 | 6 | 7 | 8)": jump_tab,
        ...
        "developer tools": open_focus_devtools,
        # Clipboard
        "cut": Key("cmd-x"),
        "copy": Key("cmd-c"),
        "paste": Key("cmd-v"),
        ...
        "go tab (named | by name)": browser.send_to_vimium("T"),
        "go tab (named | by name) <dgndictation>": [
...
def global_chrome_new_tab(m):
    switcher.switch_app(name="Breve Browser")
    press("cmd-t")


def global_go_to_webpage(m):
    switcher.switch_app(name="Brave browser")
    go_to_webpage(m)
